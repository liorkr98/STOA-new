...
- **AI vendor is DeepSeek** (`deepseek-v4-flash` by default; override with `DEEPSEEK_MODEL`), with a deterministic mock fallback when `DEEPSEEK_API_KEY` is unset (`src/lib/fact-check/claim-extraction.ts`), so local dev and CI never depend on a live key. Keys pasted into Vercel are trimmed of trailing newlines/control chars in `src/lib/ai/llm.ts`. All text LLM calls go through `llmModel()` which uses DeepSeek Chat Completions (`.chat()`), not OpenAI's Responses API.
