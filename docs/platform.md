...
- **AI vendor is DeepSeek** (`deepseek-v4-pro` by default), with a deterministic mock fallback when `DEEPSEEK_API_KEY` is unset (`src/lib/fact-check/claim-extraction.ts`), so local dev and CI never depend on a live key.
