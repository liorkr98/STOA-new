...
| [Cloudflare Stream](https://www.cloudflare.com/products/cloudflare-stream/) | **Video — the product's lead medium.** Encoding, storage, signed playback tokens | https://dash.cloudflare.com → Stream | No — block reports "provider unavailable" without keys, nothing else breaks |
...
- **Video provider is abstracted** behind `VideoProvider` (`src/lib/video/provider.ts`): Cloudflare Stream is the default (encoding included, signed playback tokens, plain HTTP API — no SDK), and Mux slots in behind the same interface when analytics matter. No component imports a provider directly; everything goes through `/api/video/upload`, `/api/video/token`, and the webhook. Env: `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_STREAM_API_TOKEN`, `CLOUDFLARE_STREAM_WEBHOOK_SECRET`. Full spec: `docs/VIDEO.md`.
