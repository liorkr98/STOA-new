...
const ROUTES: { href: string; label: string; note: string }[] = [
  ...
  { href: "/dev/compose", label: "Compose · type picker", note: "The first screen: four types by purpose and four drafts along their spines. Add ?limited=1 for the verdict window closed." },
  { href: "/dev/compose?shape=verdict", label: "Compose · verdict", note: "A verdict draft on its call step: the market-cap check, long/short, entry, target, the horizon slider, then the features menu and the visibility card on publish." },
  { href: "/dev/compose?shape=thesis", label: "Compose · thesis", note: "A thesis at its tags step with the type-to-narrow tag list, then the publish screen's features menu with cards added and a call to add." },
