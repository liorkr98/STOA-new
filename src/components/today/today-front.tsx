 ...
 * video with no still, not a publication with no video. A clip still being
 * prepared keeps the frame as well, and says so in it.
 */
export function Poster({
  ...
  return (
    <Link href={href} className="focus-ring block rounded-[var(--radius-card)]">
      <div
        className={cn(
          "relative overflow-hidden rounded-[var(--radius-card)] bg-surface-2",
          className,
        )}
      >
        {thumb.processing ? (
  ...
  return (
    <div className="num flex flex-wrap items-center gap-x-2 gap-y-1 text-[0.6875rem] uppercase tracking-[0.1em] text-text-mute">
      <Link
        ...
        className="focus-ring inline-flex items-center gap-1.5 rounded text-text"
      >
        <Avatar
        ...
        />
        <span className="font-sans text-[0.8125rem] font-semibold normal-case tracking-normal">
          {item.author.displayName}
        </span>
      </Link>
      {chips && item.ticker ? <SheetTickerChip ticker={item.ticker} /> : null}
      {chips && item.direction ? (
        <DirectionTag direction={item.direction} />
      ) : null}
      {chips && !item.ticker && item.themeTag ? (
        <span className="today-theme-chip">{item.themeTag}</span>
      ) : null}
      <span className="text-text-faint">
        {item.contentBadge.join(" · ").toUpperCase()}
      </span>
      {item.stageMarker ? (
        <span className="today-stage">{item.stageMarker}</span>
      ) : null}
      {time ? (
        <span className="text-text-faint">{sinceLabel(item.publishedAt)}</span>
      ) : null}
    </div>
  );
}
...
export function TodayLeadSplit({
  lead,
  secondary,
}: {
  lead: TodayItem;
  ...
  return (
    <section
      aria-label="The lead"
      className="mt-10 grid gap-10 md:grid-cols-[minmax(0,7fr)_minmax(0,4fr)] md:gap-12"
    >
      <article className="min-w-0">
        ...
        {lead.deck ? (
          <p className="mt-3 max-w-[60ch] font-display text-[1.0625rem] leading-relaxed text-text-mute">
            {lead.deck}
          </p>
        ) : null}
        <div className="mt-4">
      ...
      <div className="flex min-w-0 flex-col divide-y divide-[var(--border)] border-t border-[var(--border)] md:border-t-0 md:pt-0">
        {secondary.map((s, i) => (
          <article
            key={s.reportId}
            className={cn("flex gap-3", i === 0 ? "py-5 md:pt-0" : "py-5")}
          >
            <div className="min-w-0 flex-1">
              ...
              <Link
                ...
                className="focus-ring block rounded"
              >
                <h2 className="mt-1.5 font-display text-[1.375rem] font-semibold leading-[1.15] tracking-tight">
                  {s.headline}
                </h2>
              </Link>
...
export function TodayDeskRail({ items }: { items: TodayDeskItem[] }) {
  ...
  return (
    <Rail
      title="Your desk"
      note="From your memberships and follows"
      ...
      {items.map((it) => (
        <article key={it.reportId} className="w-[240px] md:w-[260px]">
          ...
          <div className="num mt-2.5 truncate text-[0.6875rem] uppercase tracking-[0.12em] text-text-mute">
            ...
            {it.relationship === "member" ? "Member" : "Following"}
          </div>
          <Link
            href={`/report/${it.reportId}`}
            className="focus-ring block rounded"
...
export function VerdictRow({ v }: { v: TodayVerdict }) {
  const seal =
    v.outcome === "hit"
      ? "hit"
      : v.outcome === "near" || v.outcome === "partial"
        ? "near"
        : "miss";
  ...
  return (
    <Rail
      ...
      aside={
        <Link
          href="/markets"
          className="band-see-all focus-ring hidden sm:inline"
        >
          The record →
        </Link>
      }
      ...
      {rest.map((it) => (
        <article key={it.reportId} className="w-[220px]">
          ...
          <Link
            ...
            className="focus-ring block rounded"
          >
            <h3 className="mt-1 font-display text-[1rem] font-semibold leading-[1.2] tracking-tight line-clamp-3">
              {it.headline}
            </h3>
          </Link>
          <div className="mt-2">
