...
import { Bell, Inbox, PenLine, Search } from "lucide-react";
  ...
  return (
    <header className="header-elevate sticky top-0 z-40 border-b border-border bg-paper pt-[var(--safe-top)] gutter-x">
      <div className="mx-auto flex h-14 w-full min-w-0 max-w-[var(--w-wide)] items-center justify-between gap-2 md:gap-4 lg:gap-6">
        ...
        {/* Mobile right cluster. No hamburger: the four destinations live in
            the bottom bar, and Settings and Sign out live in the profile area's
            Account group, which the avatar leads to. */}
            ...
            <>
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span
                    aria-hidden
                    className="absolute right-2 top-2 h-2 w-2 rounded-full bg-[var(--ink)] ring-2 ring-[var(--paper)]"
                  />
                )}
              </Link>
              <Link
                ...
                className="focus-ring ml-1 shrink-0 rounded-full"
              >
                <AvatarCircle profile={profile} />
              </Link>
            </>
